...
void UAlsAnimationInstance::SetRecoilValues(float Strength, float DelayTime,float Duration)
{
	bAddRecoilImpulse=true;

	//We can not use switch for gameplaytag.
	if (OverlayMode== AlsOverlayModeTags::M4)
	{
		RecoilValue.X=Strength;
		RecoilValue.Y=0.72f;
		RecoilValue.Z=0.8f;
	}
	else if (OverlayMode== AlsOverlayModeTags::PistolTwoHanded)
	{
		RecoilValue.X=Strength*0.7f;
		RecoilValue.Y=0.56f;
		RecoilValue.Z=0.8f;
	}
	DelegateTimerInRecoil.BindUFunction(this,"SetBoolRecoil",Duration);
	
	GetWorld()->GetTimerManager().SetTimer(
		TimerHandleInRecoil,
		DelegateTimerInRecoil,
		Duration,
		false
		);
}

void UAlsAnimationInstance::SetBoolRecoil()
{
...
void UAlsAnimationInstance::AddRecoilWhileShooting(FGameplayTag OverlayModeTag, float Strength, float DelayTime,
	float TimeBeforeStart)
{
	//Delay implementation
	DelegateTimer.BindUFunction(this,"SetRecoilValues",Strength,DelayTime,TimeBeforeStart);
	GetWorld()->GetTimerManager().SetTimer(TimerHandle, DelegateTimer, DelayTime, false);
}
...
void UAlsAnimationInstance::UpdateAimingValues()
{
	RotationDeltaBase=
		UKismetMathLibrary::RLerp(
			FRotator(0, TryGetPawnOwner()->GetActorRotation().Yaw,0 ),
			FRotator(0, TryGetPawnOwner()->GetActorRotation().Yaw+180.0f, 0),
			0.0f,
			true);
	FRotator CurrentRotation=UKismetMathLibrary::NormalizedDeltaRotator(
		TryGetPawnOwner()->GetControlRotation(),
		RotationDeltaBase
		);
	AimingAngleC=FVector2D(CurrentRotation.Pitch,CurrentRotation.Yaw);

	//Clamp the Aiming Pitch Angle to a range of 1 to 0 for use in the vertical aim sweeps.
	AimSweepTimeC=UKismetMathLibrary::MapRangeClamped(
		AimingAngleC.Y,
		-90.0f,
		90.0f,
		1.0f,
