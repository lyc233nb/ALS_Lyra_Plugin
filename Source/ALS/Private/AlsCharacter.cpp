...
void AAlsCharacter::AddRecoilAnimation()
{
	
}

void AAlsCharacter::OnBoxBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
                                      UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor && OtherActor != this)
	{
		bIsColliding = true;
		CollidingActor = SweepResult.GetActor();
		OverlapCount++;
        		/*UE_LOG(LogTemp, Warning, TEXT("Collision Started!"));*/
	}
}

void AAlsCharacter::OnBoxEndOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	if (OtherActor && OtherActor != this) {
		OverlapCount = FMath::Max(0, OverlapCount - 1);
		bIsColliding = (OverlapCount > 0);
	}
}
...
}
{
	...
	CollisionTriggerBox = CreateDefaultSubobject<UBoxComponent>(TEXT("CollisionTriggerBox"));
	CollisionTriggerBox->SetupAttachment(RootComponent); // Add to scene component
	CollisionTriggerBox->SetCollisionProfileName(TEXT("Trigger")); // Use customized collision config

	CollisionTriggerBox->SetCollisionEnabled(ECollisionEnabled::QueryOnly); 
	CollisionTriggerBox->SetGenerateOverlapEvents(true); // Enable overlap events.
...
void AAlsCharacter::BeginPlay()
{
	...
	/**
	 * remember the adddynamic function of collision trigger box should be done in the begin play, instaed of consturctor.
	 */
	CollisionTriggerBox->OnComponentBeginOverlap.AddDynamic(this, &AAlsCharacter::OnBoxBeginOverlap);
	CollisionTriggerBox->OnComponentEndOverlap.AddDynamic(this, &AAlsCharacter::OnBoxEndOverlap);
...
void AAlsCharacter::RefreshMeshProperties() const
{
	...
	if (IsValid(GetMesh())&&GetMesh()->IsUsingAbsoluteRotation() != bUseAbsoluteRotation)
	{
...
void AAlsCharacter::SetViewMode(const FGameplayTag& NewViewMode, const bool bSendRpc)
{
	if (ViewMode == NewViewMode || GetLocalRole() < ROLE_AutonomousProxy)
	{
		...
		// @Game-Change start - Add Loose Gameplay Tag(s)
...
//this setting seems to conflicts with the lyra locomotion setting, so we need to adjust the speed
	if (LocomotionState.Speed < AlsCharacterMovement->GetGaitSettings().WalkSpeed + 35.0f)
	{
