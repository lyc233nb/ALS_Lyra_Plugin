...
AActor* UAlsUtility::GetWeaponActor(ACharacter* InCharacter)
{
	ULyraEquipmentManagerComponent* EquipmentManager=InCharacter->GetComponentByClass<ULyraEquipmentManagerComponent>();
	...
	if (IsValid(EquipmentManager))
	{
		TArray<TObjectPtr<ULyraEquipmentInstance>> WeaponArray;
		WeaponArray=
			EquipmentManager->GetEquipmentInstancesOfType(ULyraWeaponInstance::StaticClass());
		if (WeaponArray.Num()>0)
		{
			if (WeaponArray[0]->GetSpawnedActors().Num()>0)
			{
				AActor* DesiredActor = WeaponArray[0]->GetSpawnedActors()[0];
