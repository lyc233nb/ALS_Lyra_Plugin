...
protected:
	...
	FTimerDelegate DelegateTimer; // Delegate to bind function with parameters
	FTimerDelegate DelegateTimerInRecoil; // Delegate to bind function with parameters
	...
	FTimerHandle TimerHandleInRecoil;
	...
	FVector2D AimingAngleC;

public:

	UPROPERTY(BlueprintReadWrite, Category = "Als|Rotation")
	double AimSweepTimeC;
	...
	virtual void AddRecoilWhileShooting(FGameplayTag OverlayModeTag, float Strength = 0, float DelayTime = 0.05, float TimeBeforeStart = 0) override;

	UFUNCTION(BlueprintCallable)
	void SetRecoilValues(float Strength, float DelayTime,float Duration);
